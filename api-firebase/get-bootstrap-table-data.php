...
if (isset($_GET['table']) && $_GET['table'] == 'users') {
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        ...
        $where .= "WHERE name like '%" . $search . "%' OR mobile like '%" . $search . "%' OR location like '%" . $search . "%'";
    }
    if (isset($_GET['sort'])){
...
if (isset($_GET['table']) && $_GET['table'] == 'functions') {
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        ...
        $where .= "WHERE function_name like '%" . $search . "%' OR date like '%" . $search . "%' OR place like '%" . $search . "%'";
    }
    if (isset($_GET['sort'])){
        $sort = $db->escapeString($_GET['sort']);
    }
    ...
    $sql = "SELECT COUNT(`id`) as total FROM `functions` ";
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];
   
    $sql = "SELECT * FROM functions " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
...
if (isset($_GET['table']) && $_GET['table'] == 'moi') {
    ...
    $sql = "SELECT COUNT(`id`) as total FROM `moi` ";
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];
